...
function extractInfo(html) {
  ...
  // Facebook pages use dynamic class names; attempt heuristic parsing.
  // Search for contact-like text blocks.
  const candidates = [];
  $('div, span, li, a').each((_, el) => {
    const text = $(el).text().trim();
    if (!text) return;
    if (/\+?\d[\d\s().-]{6,}/.test(text)) candidates.push({ kind: 'phone', value: text });
    if (/rating|stars?/i.test(text)) candidates.push({ kind: 'rating', value: text });
    if (/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/i.test(text)) candidates.push({ kind: 'email', value: text });
    if (/https?:\/\//i.test(text) || /\.com|\.org|\.net/i.test(text)) candidates.push({ kind: 'website', value: text });
  });

  for (const c of candidates) {
    if (c.kind === 'phone' && !obj.phone) obj.phone = c.value;
    if (c.kind === 'rating' && !obj.rating) obj.rating = c.value;
    if (c.kind === 'email' && !obj.email) obj.email = c.value;
    if (c.kind === 'website' && !obj.website) obj.website = c.value;
  }
...
async function scraper(targetUrl) {
  ...
  if (!targetUrl) {
    await browser.close();
    throw new Error('scraper(targetUrl) requires a non-empty URL');
  }
  ...
  return { status: resp?.status?.() || null, data: html, info };
}

// No default run: use keyword search CLI below or call scraper(url) from another file.
